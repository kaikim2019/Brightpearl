...
class Synfromapi extends \Magento\Backend\App\Action
{
    ...
    {
         $collection = $this->_objectManager->create('Bsitc\Brightpearl\Model\ResourceModel\Bpproducts\Collection');
        ...
        if ($api->authorisationToken) {
            ...
            if (count($result)>0) {
                foreach ($result['response'] as $item) {
                    ...
                    $model = $this->_objectManager->create('Bsitc\Brightpearl\Model\Bpproducts');
                    $model->setData($data);
                    $model->save();
                }
            }
