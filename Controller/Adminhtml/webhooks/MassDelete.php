...
class MassDelete extends \Magento\Backend\App\Action
{
    ...
    {
        $itemIds = $this->getRequest()->getParam('webhooks');
        if (!is_array($itemIds) || empty($itemIds)) {
            $this->messageManager->addError(__('Please select item(s).'));
        } else {
            try {
                foreach ($itemIds as $itemId) {
                    $post = $this->_objectManager->get('Bsitc\Brightpearl\Model\Webhooks')->load($itemId);
                    /* ----------- delete web hook ---------------------*/
                        $api = $this->_objectManager->create('Bsitc\Brightpearl\Model\Api');
                    if ($api->authorisationToken) {
                        $api->deleteWebhook($model->getWebhookId());
                    }
                    /* ----------- delete web hook ---------------------*/
                    $post->delete();
                }
